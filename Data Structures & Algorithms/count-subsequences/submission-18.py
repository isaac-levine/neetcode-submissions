class Solution:
    def numDistinct(self, s: str, t: str) -> int:
        ...
        #       1
        m, n = len(s), len(t)
        # dp = [[0] * (n + 1) for _ in range(m + 1)] # i think the rows go on the more outer thing...its been a while since i did 2d
        # for r in range(m + 1):
        #     dp[r][n] = 1
        dp = [0] * (n + 1)
        dp[n] = 1
        ...
        for r in range(m - 1, -1, -1):
            for c in range(n): # now we have to go forwards to make sure when we read diagonal we're reading old stuff and not new 
                if s[r] == t[c]:
                    dp[c] += dp[c + 1]
