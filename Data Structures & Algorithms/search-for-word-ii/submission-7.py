...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        head = TrieNode(None)
        # build a trie from the words so that we can more efficiently check if our current path is a valid word
        for w in words:
            cur = head
            for c in w:
        ...
        def dfs(r, c, cur_word, trie, path):
            ...
            if trie.endOfWord:
                completed_words.add(cur_word) # found a word does not mean done in this path... could be "back" and "backer"
            ...
            for dr, dc in directions:
                dfs(r + dr, c + dc, cur_word, trie, path)
            path.remove((r, c))

        for r in range(ROWS):
            for c in range(COLS):
                if board[r][c] in head.children:
                    dfs(r, c, "", head, set()) # go find the word
